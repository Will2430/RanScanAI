"""
Train production-ready Random Forest model for Zenodo dataset
Removes data leakage features and saves to models directory
Matches XGBoost/GB training pipeline for fair comparison
"""
...
leakage_columns = [
    ...
    'file_extension', 'FileExtension', 'extension', 'Extension',
    'processes_malicious', 'processes_suspicious', 
...
for field in characteristics_fields:
    if field in X.columns:
        if X[field].dtype == 'object':
            ...
            # Numeric format: convert to flag count
            logger.info(f"  Converting bitmask field '{field}' to flag count")
            X[field] = X[field].apply(count_flags)

# NORMALIZE ImageBase
if 'ImageBase' in X.columns and 'PEType' in X.columns:
    logger.info(f"  Normalizing ImageBase based on PEType")
...
# Scale features (for consistency, though RF doesn't strictly need it)
...
model_filename = f'random_forest_zenodo_{timestamp}.pkl'
scaler_filename = f'scaler_rf_{timestamp}.pkl'
features_filename = f'features_rf_{timestamp}.json'
metadata_filename = f'random_forest_metadata_{timestamp}.json'
...
print("\n" + "=" * 80)
print("✅ RANDOM FOREST MODEL READY FOR ENSEMBLE")
print("=" * 80)
...
print(f"   ✓ Ensemble voting with GB + XGBoost")
print(f"   ✓ Multi-model comparison analysis")

print("\n" + "=" * 80)
print("NEXT STEPS:")
print("=" * 80)
print("1. Run build_ensemble_voting.py to create ensemble classifier")
print("2. Run multi_model_benchmark.py for comparative analysis")
