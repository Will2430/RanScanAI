"""
Train production-ready XGBoost model for Zenodo dataset
Removes data leakage features and saves to models directory
Third model for multi-model comparison and ensemble voting
"""
import pandas as pd
import numpy as np
...
# Scale features (XGBoost benefits from scaling for better convergence)
...
model_filename = f'xgboost_zenodo_{timestamp}.pkl'
scaler_filename = f'scaler_xgb_{timestamp}.pkl'
features_filename = f'features_xgb_{timestamp}.json'
metadata_filename = f'xgboost_metadata_{timestamp}.json'
...
print("\n" + "=" * 80)
print("✅ XGBOOST MODEL READY FOR ENSEMBLE")
print("=" * 80)
...
print(f"   ✓ Ensemble voting with RF + GB")
