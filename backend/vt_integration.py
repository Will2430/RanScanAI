...
class VirusTotalEnricher:
    ...
    def upload_file_for_scan(self, file_path: str) -> Optional[Dict[str, Any]]:
        """
        Upload file to VirusTotal for multi-AV scanning
        
        WARNING: Uploaded files become PUBLIC and are shared with security community
        
        Uses v3 API: POST /files
        Returns scan_id for later result retrieval
        
        Args:
            file_path: Path to file to upload
            
        Returns:
            Dict with scan_id and initial data, or None if failed
        """
        file_path_obj = Path(file_path)
        ...
        if file_size > 32 * 1024 * 1024:
            logger.error(f"File too large: {file_size / 1024 / 1024:.1f}MB (max 32MB)")
            return None
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f" File uploaded to VT: {scan_id}")
                return {
                    'scan_id': scan_id,
                    'uploaded': True,
                    'data': data
            ...
            logger.error(f"Failed to upload file to VT: {e}")
            return None
    
    def get_scan_results(self, scan_id: str, wait: bool = True, max_wait_seconds: int = 300) -> Optional[Dict[str, Any]]:
        """
        Get scan results for a previously uploaded file
        
        Args:
            scan_id: Scan ID from upload_file_for_scan()
            wait: If True, poll until scan completes (default: True)
            max_wait_seconds: Maximum time to wait for results (default: 300s = 5 minutes)
            
        Returns:
            Dict with detection results from all AV engines, or None if failed
        """
        url_analyses = f"{self.VT_API_BASE}/analyses/{scan_id}"
        headers = {'x-apikey': self.api_key}
        
        max_attempts = (max_wait_seconds // 5) if wait else 1  # Poll every 5 seconds
        
        for attempt in range(max_attempts):
            try:
                self.rate_limiter.wait_if_needed()
                response = requests.get(url_analyses, headers=headers, timeout=30)
                ...
                if response.status_code == 200:
                    ...
                    if status == 'completed':
                        ...
                        logger.info(f"✓ VT scan complete: {stats.get('malicious', 0)}/{sum(stats.values())} detections")
                        ...
                        return {
                            ...
                            'scans': results,  # Full AV verdicts
                            'stats': stats
                        }
                    elif status == 'queued' or status == 'in-progress':
                        if attempt < max_attempts - 1:
                            logger.debug(f"VT scan {status}, waiting... ({attempt+1}/{max_attempts})")
                            time.sleep(5)
                            continue
                        else:
                            logger.warning(f"VT scan timeout after {max_wait_seconds}s")
                            return None
                else:
                    logger.error(f"VT API error {response.status_code}: {response.text}")
                    return None
                ...
                logger.error(f"Failed to get VT scan results: {e}")
                return None
