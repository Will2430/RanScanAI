...
# Handle both regular Python execution and PyInstaller bundled EXE
if getattr(sys, 'frozen', False):
    # Running as PyInstaller EXE - use fallback
    print("[INIT] Running as bundled EXE - using fallback path configuration")
    def get_test_folder(prefer_downloads=False):
        """Fallback path configuration for bundled EXE"""
        # Use a path relative to current working directory or a fixed location
        test_folder = Path(os.getcwd()) / "RANSOMWARE_TEST_FOLDER"
        if not test_folder.exists() or "OneDrive" not in str(test_folder):
            # Fallback to Downloads
            test_folder = Path.home() / "Downloads" / "RANSOMWARE_TEST_FOLDER"
        return test_folder
else:
    # Running as Python script - use dynamic path config
    # dynamic_path_config lives in testing_code/, which is parent.parent from here
...
# ============================================================================
TEST_FOLDER = get_test_folder(prefer_downloads=False)  # Dynamically determined test folder
...
# ============================================================================
# REAL CRYPTO HELPERS  (BCryptGenRandom fires the Frida hook; AES-256-CBC does
# the actual encryption so files are genuinely encrypted, not just renamed)
# ============================================================================

def bcrypt_gen_random(size: int) -> bytes:
    """Generate random bytes using Windows BCryptGenRandom.
    Calling this function fires the Frida BCryptGenRandom hook (crypto_ops++)."""
    try:
        _bcrypt = ctypes.windll.bcrypt
        BCRYPT_USE_SYSTEM_PREFERRED_RNG = 0x00000002
        buf = ctypes.create_string_buffer(size)
        status = _bcrypt.BCryptGenRandom(None, buf, size, BCRYPT_USE_SYSTEM_PREFERRED_RNG)
        if status == 0:
            return bytes(buf)
    except Exception:
        pass
    return os.urandom(size)  # fallback if BCrypt fails


def aes256_encrypt(plaintext: bytes, key: bytes, iv: bytes) -> bytes:
    """Encrypt plaintext with AES-256-CBC. Returns raw ciphertext (IV is prepended by caller)."""
...
def simulate_file_encryption():
    ...
    # Real AES-256-CBC encryption using BCryptGenRandom for key/IV material
    # -----------------------------------------------------------------------
    # Generate master 256-bit key once via BCryptGenRandom (fires Frida hook)
    ENCRYPTION_KEY = bcrypt_gen_random(32)
    print(f"[FILE] Generated AES-256 master key via BCryptGenRandom ({len(ENCRYPTION_KEY)*8}-bit)")
    ...
    for i, filepath in enumerate(created_files):
        try:
            ...
            # Per-file 128-bit IV via BCryptGenRandom (fires Frida hook each time)
            iv = bcrypt_gen_random(16)

            # Actual AES-256-CBC encryption
            ciphertext = aes256_encrypt(content, ENCRYPTION_KEY, iv)

            # Write IV + ciphertext back (IV prepended so decryption is possible)
            filepath.write_bytes(iv + ciphertext)
    ...
    print(f"[FILE] Really encrypted {len(encrypted_files)} files with AES-256-CBC")
